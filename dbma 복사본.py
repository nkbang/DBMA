...
def calculate_noise_score(text: str) -> dict:
    """
    다국어 노이즈 분석 — 한국어·영어·히브리어·헬라어(고대·현대) 지원.

    지원 문자 범주:
        - 한국어  : 가-힣 (완성형 한글)
        - 영어    : a-zA-Z
        - 히브리어: \u05d0-\u05ea (기본 블록) + \u05b0-\u05c7 (니쿠드/강세)
                    + \ufb1d-\ufb4e (표현형)
        - 헬라어  : \u0370-\u03ff (그리스 블록) + \u1f00-\u1fff (확장/기식부호)
                    ← 고대 헬라어 강세·기식 부호 포함
        - 공통    : 숫자·공백·기본 구두점

    반환값 dict:
        total        : 종합 노이즈 점수 (0=완벽, 100=최악)
        symbol_ratio : 비허용 특수문자 비율 점수 (0~40)
        short_line   : 짧은 줄 비율 점수         (0~30)
        blank_ratio  : 공백 줄 비율 점수          (0~15)
        repeat_ratio : 반복 패턴 점수             (0~15)
        char_count   : 전체 문자 수
        line_count   : 전체 줄 수
        word_count   : 단어 수
        lang_detected: 감지된 주요 언어 목록
    """
    if not text or len(text) < 50:
        return {"total":100.0,"symbol_ratio":40.0,"short_line":30.0,
                "blank_ratio":15.0,"repeat_ratio":15.0,
                "char_count":len(text),"line_count":1,"word_count":0,
                "lang_detected":[]}

    # ── 유니코드 NFC 정규화 (히브리·헬라 결합문자 안정화) ──────────────────
    ...
    # ── ① 비허용 특수문자 비율 (가중치 40) ───────────────────────────────
    # 허용 범주: 한·영·히브리·헬라 문자, 숫자, 공백, 기본 구두점,
    #            히브리 니쿠드(\u05b0-\u05c7), 헬라 기식/강세(\u1f00-\u1fff)
    ALLOWED = re.compile(
        r'['
        r'가-힣'                      # 한국어
        r'a-zA-Z'                     # 영어
...
def noise_color(score: float) -> str:
    if score < 20:  return "#3fb950"   # 초록 — 우수
    if score < 40:  return "#d29922"   # 주황 — 주의
    return "#f85149"                   # 빨강 — 재정제 필요

def noise_label(score: float) -> str:
    if score < 20:  return "우수 ✓"
...
def load_chunks_info(directory: str, stem: str) -> list[str] | None:
    """stem_chunks.txt 가 있으면 청크 목록 반환, 없으면 None."""
...
def _on_item_change():
    ...
    if sel_keys:
        st.session_state["_select_all"] = all(st.session_state[k] for k in sel_keys)
...
with tab1:
    ...
    with col_left:
        ...
        if not file_list:
            ...
            # 전체 선택
            st.checkbox("전체 선택 / 해제", key="_select_all", on_change=_apply_select_all)
            ...
            for f in file_list:
                ...
                with c1:
                    st.checkbox(
                        label="", key=f"sel_{f['name']}",
                        on_change=_on_item_change, label_visibility="collapsed",
                    )
                with c2:
                    st.markdown(f"""
                    <div class="file-row">
        ...
        if start_btn and selected_files:
            ...
            if use_ocr:
                ...
                pipeline_options.ocr_options = EasyOcrOptions(
                    lang=["ko", "en", "he", "el"],  # 한국어·영어·히브리어·헬라어
                    use_gpu=False,
                )
            else:
            ...
            if use_ocr:
                logs.append({"cls":"log-info","msg":"[OCR] EasyOCR 활성화 — ko · en · he (히브리) · el (헬라)"})
            render_logs(logs)
...
with tab2:
    ...
    if not md_files:
        ...
        for x in sorted(all_scores, key=lambda v: v["noise"]["total"], reverse=True):
            ...
            col = noise_color(sc)
            bar_pct = int(sc)
            # 언어 감지는 저장된 MD 프론트매터에서 읽음
            with open(x['path'], encoding='utf-8') as _f:
                _fm = _f.read(500)
            _lm = __import__('re').search(r'languages: *(.+)', _fm)
        ...
        if selected_md:
            ...
            with st.expander("📄 MD 본문 전체 보기"):
                ...
                st.text_area("", body[:3000] + ("…" if len(body) > 3000 else ""),
