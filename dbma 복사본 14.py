...
def detect_repeated_lines(lines, min_repeat=5):
    candidates = []
    for line in lines:
        line = line.strip()
        if len(line) < 5:
    ...
    repeated = {line for line, count in freq.items() if count >= min_repeat}
    return repeated


def remove_headers_footers(text, aggressive=False):
    if not aggressive:
        return text

    lines = text.splitlines()
    repeated = detect_repeated_lines(lines, min_repeat=5)
    cleaned = []
...
def remove_page_numbers(text):
    ...
    for line in lines:
        stripped = line.strip()
        is_page = any(re.match(p, stripped) for p in PAGE_PATTERNS)
        if is_page:
            continue
        cleaned.append(line)
...
def estimate_footnote_ratio(text):
    lines = text.splitlines()
    footnote_lines = sum(
        1
        for line in lines
    ...
    return round(footnote_lines / max(len(lines), 1), 4)


def clean_markdown(text, aggressive_headers=False):
    text = unicodedata.normalize("NFC", text)
    text = remove_headers_footers(text, aggressive=aggressive_headers)
...
def calculate_noise_score(text: str) -> dict:
    """
    Gopher(DeepMind, Rae et al. 2021) / C4(Google) 계열 텍스트 품질 휴리스틱을
    4개 지표로 단순화해 적용. 각 지표는 LLM 학습 코퍼스 정제 파이프라인
    (RedPajama, Dolma 등)에서 실제 쓰이는 규칙을 0~100 스케일로 재매핑한 것.
      - symbol_ratio    (cap 40) : 기호/단어 비율 > 0.1 이면 만점 (Gopher 규칙)
      - word_len_anomaly(cap 25) : 평균 단어 길이가 정상범위[2,12]를 벗어난 정도
      - dup_line_ratio  (cap 20) : 중복(반복) 줄 비율 > 0.3 이면 만점 (Gopher 규칙)
      - repeat_ratio    (cap 15) : zlib 압축비가 비정상적으로 낮으면(=반복 심함) 가점
    """
    if not text or len(text) < 50:
        return {
            "total": 100.0,
            "symbol_ratio": 40.0,
            "word_len_anomaly": 25.0,
            "dup_line_ratio": 20.0,
            "repeat_ratio": 15.0,
            "char_count": len(text),
            "line_count": 1,
    ...
    # ── ① 기호/단어 비율 (Gopher symbol-to-word ratio rule, 임계값 0.1) ──
    allowed = re.compile(
        r"["
        r"가-힣"
        r"a-zA-Z"
    ...
    symbol_count = char_count - allowed_count
    symbol_to_word = symbol_count / max(word_count, 1)
    sym_score = min(symbol_to_word / 0.1, 1.0) * 40.0

    # ── ② 평균 단어 길이 이상치 (Gopher mean-word-length rule, 정상범위 2~12자) ──
    mean_word_len = (sum(len(w) for w in words) / word_count) if word_count else 0.0
    deviation = max(0.0, 2.0 - mean_word_len, mean_word_len - 12.0)
    wl_score = min(deviation / 3.0, 1.0) * 25.0

    # ── ③ 중복 줄 비율 (Gopher/C4 duplicate-line rule, 임계값 0.3) ──
    non_blank = [ln.strip() for ln in lines if ln.strip()]
    total_nb = len(non_blank) or 1
    freq = Counter(non_blank)
    dup_line_count = sum(c for c in freq.values() if c >= 2)
    dup_ratio = dup_line_count / total_nb
    dup_score = min(dup_ratio / 0.3, 1.0) * 20.0

    # ── ④ 압축비 기반 반복성 (zlib deflate; 자연어는 통상 30~45%대로 압축됨) ──
    raw_bytes = text.encode("utf-8", errors="ignore")
    compressed = zlib.compress(raw_bytes, level=6)
    comp_ratio = len(compressed) / max(len(raw_bytes), 1)
    BASELINE, FLOOR = 0.32, 0.20
    rp_score = min(max(BASELINE - comp_ratio, 0.0) / (BASELINE - FLOOR), 1.0) * 15.0

    total = sym_score + wl_score + dup_score + rp_score
    return {
        "total": round(total, 2),
        "symbol_ratio": round(sym_score, 2),
        "word_len_anomaly": round(wl_score, 2),
        "dup_line_ratio": round(dup_score, 2),
        "repeat_ratio": round(rp_score, 2),
        "char_count": char_count,
        "line_count": line_count,
        "word_count": word_count,
        "lang_detected": lang_detected,
        "footnote_ratio": estimate_footnote_ratio(text),
    }
...
def score_md_file(path: str, mtime: float) -> dict:
    """mtime이 바뀌지 않으면 재계산하지 않음 — 품질 분석 탭 위젯 조작 시 전체 재스캔 방지."""
    with open(path, encoding="utf-8") as fh:
        content = fh.read()
    return calculate_noise_score(content)
...
def append_result_csv(output_dir: str, row: dict):
    ...
    columns = [
        ...
        "status", "noise_score", "chunk_count", "footnote_ratio", "error"
    ]
...
def save_md(output_dir: str, stem: str, text: str, noise: dict) -> str:
    ...
    header = (
        ...
        f"footnote_ratio: {noise.get('footnote_ratio', 0.0)}\n"
...
def save_json(output_dir: str, stem: str, chunks: list[str], metadata: dict) -> str:
    """JSON 형식으로 저장"""
...
@st.cache_resource(show_spinner="OCR 엔진 로딩 중... (최초 1회만 소요됩니다)")
...
def run_parsing_batch(
    ...
    logs.append({"cls": "log-info", "msg": f"[클리너] page/hyphen/ocr/space 정제 활성화"})
    if aggressive_headers:
        logs.append({"cls": "log-warn", "msg": "[클리너] Header/Footer 보수 제거 활성화"})
    if use_ocr:
    ...
    for idx, f in enumerate(file_batch):
        ...
        try:
            ...
            cleaned_text = clean_markdown(raw_text, aggressive_headers=aggressive_headers)
            noise = calculate_noise_score(cleaned_text)
            lang_list = detect_langs(cleaned_text)
            lang_found = " · ".join(
            ...
            logs.append({"cls": "log-info", "msg": f"  ◎ 각주 비율: {noise.get('footnote_ratio', 0.0):.4f}"})

            if len(raw_text) != len(cleaned_text):
                reduced = len(raw_text) - len(cleaned_text)
                logs.append({"cls": "log-info", "msg": f"  ◎ 클리너 적용: {reduced:,}자 정리"})

            md_path = save_md(output_dir, stem, cleaned_text, noise)
            ...
            # JSON 파일 저장 (선택적)
            if save_json_format:
                json_path = save_json(output_dir, stem, chunks, noise)
                logs.append({"cls": "log-ok", "msg": f"  ✓ JSON → {os.path.basename(json_path)}"})
            ...
            append_result_csv(output_dir, {
                ...
                "footnote_ratio": noise.get("footnote_ratio", 0.0),
                "error": "",
            })
    ...
    logs.append({"cls": "log-info", "msg": f"[산출물] .md + _chunks.txt + .json → {output_dir}"})
...
###
    ...
    if not target_dir or not isinstance(target_dir, str):
        path_errors.append("RAW 폴더 경로가 유효하지 않습니다.")
    elif not os.path.exists(target_dir):
        path_errors.append(f"RAW 폴더 경로가 존재하지 않습니다: {target_dir}")
        
    if not output_dir or not isinstance(output_dir, str):
        path_errors.append("완성본 폴더 경로가 유효하지 않습니다.")
    elif not os.path.exists(output_dir):
        path_errors.append(f"완성본 폴더 경로가 존재하지 않습니다: {output_dir}")
    ...
    if not isinstance(chunk_size, int) or chunk_size <= 0:
        chunk_errors.append("Chunk Size는 양의 정수여야 합니다.")
        
    if not isinstance(chunk_overlap, int) or chunk_overlap < 0:
        chunk_errors.append("Chunk Overlap는 음수가 아닌 정수여야 합니다.")
        
    if chunk_overlap >= chunk_size:
        chunk_errors.append("Chunk Overlap는 Chunk Size보다 작아야 합니다.")
    ...
    use_ocr = st.checkbox(
        "OCR 활성화 (스캔본 PDF 전용)",
        value=False,
        help="디지털 PDF는 해제. 스캔 이미지 PDF만 활성화합니다. "
             "EasyOCR은 한국어, 영어, 히브리어, 헬라어를 지원합니다."
    )
    if use_ocr:
        st.info("EasyOCR — 한국어 · 영어 · 히브리어 · 헬라어")

    st.markdown("---")
    st.markdown("### 🧽 Cleaner 설정")
    aggressive_headers = st.checkbox(
        "Header/Footer 보수 제거",
        value=False,
        help="반복 줄 제거를 시도합니다. "
             "오탐 가능성 때문에 기본값은 비활성화입니다. "
             "이 기능을 활성화하면 일부 문서가 잘못 정리될 수 있습니다."
    )

    backup_mode = st.checkbox(
        "재파싱 시 기존 산출물 백업",
        value=True,
        help="같은 파일을 다시 파싱할 때 기존 md/chunks 결과를 bak 파일로 백업합니다."
    )

    # JSON 저장 옵션 추가
    save_json_format = st.checkbox("JSON 형식으로도 저장", value=False)
    if save_json_format:
        st.info("파싱 완료 후 MD, TXT, JSON 모두 생성됩니다.")
...
with tab1:
        ...
        if not file_list:
            ...
            for f in file_list:
                ...
                with c1:
                    ...
                    st.checkbox(label="", key=key, label_visibility="collapsed")
            ...
            if selected_files:
                ...
                st.markdown(
                    ...
                    f"font-family:JetBrains Mono,monospace;'>✓ {len(selected_files)}개 선택됨 · {fmt_size(sel_size)}</div>",
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    "<div style='font-size:0.77rem;color:#d29922;padding:7px 4px;"
                    "font-family:JetBrains Mono,monospace;'>선택된 파일이 없습니다.</div>",
                    unsafe_allow_html=True,
                )
    ...
    with col_right:
        ...
        # 경로 유효성 검사
        if path_errors:
            st.warning("경로가 유효하지 않습니다. 먼저 경로를 수정해주세요.")
        elif chunk_errors:
            st.warning("청크 설정이 유효하지 않습니다. 다시 확인해주세요.")
        else:
            st.markdown(f"""
            <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;
              ...
              <span style="color:#8b949e;">CLEANER</span> <span style="color:#3fb950;">활성</span><br>
              <span style="color:#8b949e;">선택 파일</span> <span style="color:#58a6ff;">{n_sel}개</span>
            </div>
            """, unsafe_allow_html=True)

            start_btn = st.button(f"🚀 파싱 시작 ({n_sel}개)", disabled=(not file_list or n_sel == 0))
            st.markdown("---")
            log_ph = st.empty()
            prog_ph = st.empty()
            ...
            if start_btn and selected_files:
                result = run_parsing_batch(
                    ...
                    log_ph=log_ph,
                    prog_ph=prog_ph,
                    aggressive_headers=aggressive_headers,
...
with tab2:
    ...
    if not md_files:
        ...
        all_scores = []
        for mf in md_files:
            mtime = os.path.getmtime(mf["path"])
            ns = score_md_file(mf["path"], mtime)
        ...
        st.markdown("#### 📁 파일별 품질 상태")
        for x in sorted(all_scores, key=lambda v: v["noise"]["total"], reverse=True):
            sc = x["noise"]["total"]
            col = noise_color(sc)
            bar_pct = int(min(sc, 100))
            langs = ", ".join(x["noise"].get("lang_detected", [])) or "미감지"
        ...
        with cola:
            ...
            selected_md = st.selectbox("상세 분석 파일 선택", filenames)
            ...
            <div class="analysis-card">
              ...
              <div style="font-size:0.82rem;color:{col};margin-top:4px;">
                {noise_label(sc)}
              </div>
              <div class="noise-bar-wrap" style="margin-top:12px;">
                <div class="noise-bar" style="width:{int(min(sc,100))}%;background:{col};"></div>
              </div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:0.71rem;
                          color:#8b949e;margin-top:10px;line-height:1.9;">
                {ns["char_count"]:,} chars<br>
                {ns["word_count"]:,} words<br>
                {ns["line_count"]:,} lines<br>
                {", ".join(ns.get("lang_detected", [])) or "언어 미감지"}
              </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with colb:
            def mini_bar(label, val, max_val, color):
                ...
                <div style="margin-bottom:10px;">
                  <div style="display:flex;justify-content:space-between;
                    ...
                    <span>{label}</span><span style="color:{color};">{val:.1f} / {max_val}</span>
                  </div>
                  <div class="noise-bar-wrap">
                    <div class="noise-bar" style="width:{pct}%;background:{color};"></div>
                  </div>
                </div>
                """
            ...
            <div class="analysis-card">
              <h4>노이즈 세부 구성</h4>
              {mini_bar("기호/단어 비율", ns["symbol_ratio"], 40, "#f85149")}
              {mini_bar("단어길이 이상치", ns["word_len_anomaly"], 25, "#d29922")}
              {mini_bar("중복 줄 비율", ns["dup_line_ratio"], 20, "#58a6ff")}
              {mini_bar("압축비 반복성", ns["repeat_ratio"], 15, "#a371f7")}
            </div>
            """, unsafe_allow_html=True)

        st.markdown("---")

        with st.expander("MD 내용 미리보기", expanded=False):
        ...
        if chunks:
            ...
            <div class="analysis-card">
              <h4>청크 미리보기 <span style="color:#8b949e;font-size:0.75rem;
                  font-family:'JetBrains Mono',monospace;font-weight:400;">총 {len(chunks)}개</span></h4>
            </div>
            """, unsafe_allow_html=True)

            preview_n = st.slider("미리 볼 청크 수", 1, min(len(chunks), 10), 3)
            ...
            st.info("_chunks.txt 파일이 없거나 읽을 수 없습니다.")

        st.markdown("---")
        st.markdown("#### ♻️ 재파싱")

        source_pdf = extract_source_pdf_from_md(sel_data["path"])
        if source_pdf:
            resolved_pdf_path = resolve_pdf_path(source_pdf, target_dir, output_dir)
            st.markdown(f"""
            <div class="reparse-row">
              source: {source_pdf}<br>
              resolved: {resolved_pdf_path if resolved_pdf_path else "찾을 수 없음"}
            </div>
            """, unsafe_allow_html=True)

            reparse_col1, reparse_col2 = st.columns([1, 1])
            with reparse_col1:
                reparse_with_ocr = st.checkbox("재파싱 시 OCR 사용", value=use_ocr, key="reparse_ocr")
            with reparse_col2:
                reparse_aggressive = st.checkbox("재파싱 시 Header/Footer 제거", value=aggressive_headers, key="reparse_clean")

            if st.button("선택 파일 재파싱", disabled=(resolved_pdf_path is None)):
                log_ph = st.empty()
                prog_ph = st.empty()

                file_batch = [{
                    "path": resolved_pdf_path,
                    "name": os.path.basename(resolved_pdf_path),
                    "size_b": os.path.getsize(resolved_pdf_path),
                    "size_str": fmt_size(os.path.getsize(resolved_pdf_path)),
                    "mtime": datetime.datetime.fromtimestamp(os.path.getmtime(resolved_pdf_path)).strftime("%Y-%m-%d %H:%M"),
                }]

                result = run_parsing_batch(
                    file_batch=file_batch,
                    output_dir=output_dir,
                    use_ocr=reparse_with_ocr,
                    chunk_size=int(chunk_size),
                    chunk_overlap=int(chunk_overlap),
                    log_ph=log_ph,
                    prog_ph=prog_ph,
                    aggressive_headers=reparse_aggressive,
                    backup_mode=backup_mode,
                    save_json_format=save_json_format,
                )
                ...
                if result["fail_count"] == 0:
                    ...
                    st.warning(f"⚠️ 재파싱 실패 {result['fail_count']}건")
        else:
            st.info("MD 메타데이터에서 source PDF를 찾지 못했습니다.")
...
def check_dependencies():
    ...
    if missing_packages:
        st.error(f"필수 패키지가 설치되지 않았습니다: {', '.join(missing_packages)}")
        st.info("다음 명령어로 설치해주세요: pip install -r requirements.txt")
        return False
    return True
