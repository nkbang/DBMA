...
.file-table-header {
    display: grid;
    grid-template-columns: 40px 1fr 90px 150px 80px;
    background: #161b22;
    padding: 9px 12px;
    font-size: 0.70rem;
    color: #8b949e;
...
<div class="dbma-header">
    <h1>DBMA</h1>
    <p>David Bang Ministry Archive · RAG 데이터 정제 시스템 v3.1</p>
</div>
""",
    unsafe_allow_html=True,
)
    ...
    for ext in SUPPORTED_EXTENSIONS:
        ...
        for fp in glob.glob(os.path.join(directory, pattern)):
            if not os.path.isfile(fp):
                continue
            s = os.stat(fp)
            ext_only = os.path.splitext(fp)[1].lower().replace(".", "")
            files.append(
                {
                    "path": fp,
...
""",
            ...
            for f in file_list:
                ...
                with c1:
                    st.checkbox("label", key=f"sel_{f['name']}", onchange=on_item_change, label_visibility="collapsed")
                with c2:
...
<div class="file-row">
    ...
    <span class="fbadge">{file_type}</span>
</div>
""",
                        unsafe_allow_html=True,
                    )
...
""",
        ...
        if start_btn and selected_files:
            ...
            for idx, f in enumerate(selected_files):
                ...
                logs.append({"cls": "log-info", "msg": f"[{idx+1}/{total}] {name}"})
                render_logs(logs)

                try:
                    full_text = extract_text_from_file(src_path, converter=converter)
                    if not full_text.strip():
                        logs.append({"cls": "log-warn", "msg": f"{name} 추출 결과가 비어 있습니다."})
                        fail_count += 1
                        render_logs(logs)
                        continue
                    ...
                    logs.append({"cls": "log-err", "msg": f"{name} 오류: {e}"})
                    fail_count += 1
