...
## 릴레이 34 — Task Order 049: UX-007 §9 Empty/Loading/Error States (2026-08-19, 현재 유효)
...
지금부터 아래 작업 명령서를 열어서 그대로 수행하라.
  ...
  docs/agents/c1/C1-TASK-ORDER-049.md

핵심 규칙:
- 장시간 무인 작업이다. 질문하지 말고, 승인을 기다리지 마라.
- 1순위: st.error(f"...{e}") / f"...{str(e)}" 형태로 원시 예외를 화면에
  그대로 보여주는 곳(문서 §1에 9곳 예시, 전수 조사는 네가 직접) —
  "무엇을 다시 하면 되는지" 안내 문장으로 바꾸고, 원본 예외는
  logger.exception()/logger.error()로 로그에만 남겨라. 파일에 logger가
  없으면 import logging + logger = logging.getLogger(__name__) 추가해라
  (research.py에 이미 있는 패턴 참고).
- 2순위: library.py의 "아직 처리되지 않은 문서입니다" 메시지에 "자료
  등록으로 이동" 버튼을 추가하되, NAE_ADMIN_MODE=1일 때만 보여라 —
  Processing 페이지 자체가 일반 사용자에겐 안 보이는 관리자 전용이라
  (Task Order 041), 그 외엔 버튼 없이 "곧 준비됩니다" 수준 문구만 유지.
- 3순위: 문서 §3의 st.info/st.warning 목록을 각각 검토해서 "버튼
  불필요(이미 다음 행동이 명확하거나 성공 상태)" / "진짜 Dead End라
  버튼 추가"로 분류해라 — 억지로 버튼을 붙이지 마라, 판단 근거를
  보고서 표에 남겨라.
- 4순위(수정 예상 안 함): 새 로딩 UI 패턴이 있는지만 grep으로 확인하고
  없으면 "위반 없음"으로 보고서에 적어라.
- Task Order 040~048에서 이미 완료된 기능 로직(설교 연구/인용 카드/
  AI 답변/사이드바)은 건드리지 마라 — 이번엔 오류 메시지/빈 화면
  문구만.
- 완료 조건(문서 §6)을 실측으로 확인해라. AppTest로 관련 화면 전부
  렌더 확인하고, pytest tests/ 전체(부분 배치 금지 — 이번이 몇 번째
  지시인지 알지? 반드시 지켜라)를 돌려서 결과를 그대로 붙여넣어라.
- docs/agents/c1/C1-TASK-ORDER-049-REPORT.md 작성하고 끝내라.
...
## 릴레이 33 — Correction Order 048: Task Order 048 반려, 크래시 2건 (2026-08-19, 완료 — 참고용)
