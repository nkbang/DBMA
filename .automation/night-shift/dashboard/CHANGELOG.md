...
| 버전 | 커밋 | 날짜 | 내용 |
|---|---|---|---|
| **0.8.0** | `(this)` | 2026-09-16 | TSU Human Review 패널 추가 — NAE-TSU-REVIEW-WORKFLOW 대상 3개 소스(Dagg_Church_Order/Hiscox_Standard_Manual/Fuller_Complete_Works_Vol08)의 `review_status`(verified/rejected/generated) 집계를 소스별로 읽기 전용 표시, 전체 진행률 바 + COMPLETE/IN PROGRESS 배지. tsu.json(수 MB)을 5초마다 전량 재파싱하지 않도록 파일 mtime 기준 캐시 적용. 백엔드 `collector.TSU_REVIEW_IDENTIFIERS`, `monitor_state._compute_tsu_review_snapshot()`(+`tsu_review` 필드), 프런트 `TsuReviewPanel.vue` |
| **0.7.0** | `cc1308ee` | 2026-09-14 | CJK 표기 오류 복구 패널 추가 — `scripts/nae_fuller_cjk_reextract.py`(Dagg_Church_Order/Hiscox_Standard_Manual)의 running/complete 상태와 완료 시 `cjk_reextract_report.json` 요약(candidates/repaired/reextracted/residual/failed/elapsed)을 읽기 전용으로 표시. 백엔드 `collector.parse_active_cjk_repair_identifiers()`(ps grep) + `CJK_REPAIR_WATCH_IDENTIFIERS`, `monitor_state.cjk_repair` 필드, 프런트 `CjkRepairPanel.vue` |
| **0.6.0** | `2819622` | 2026-09-08 | 체크포인트 사이 진행률 보간 — `tsu_report.json` 이 100건마다(~16분) 갱신돼 숫자가 멈춰 보이던 문제. `working` 상태에서 마지막 체크포인트 이후 경과·관측 rate로 다음 체크포인트 직전까지 추정. `processed_is_estimate` + `checkpoint_evaluated` 노출, 프런트에 `≈` 표기·해치 바·"estimated between checkpoints" |
