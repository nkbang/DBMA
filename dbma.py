...
def get_cache_stats():
    ...
    return {"count": len(all_items), "bytes": total_bytes, "mb": round(total_bytes / (1024 * 1024), 2), "md_count": len(groups["md"]), "chunk_count": len(groups["chunks"]), "other_count": len(groups["other"]), "md_bytes": md_bytes, "chunk_bytes": chunk_bytes, "other_bytes": other_bytes, "items": sorted(all_items, key=lambda x: x["size"], reverse=True)}
...
def process_one_file(path: Path, use_ocr: bool, chunk_size: int, chunk_overlap: int):
    try:
        # --- 기존 추출·언어감지·노이즈 로직은 그대로 유지 ---
        # converter = DocumentConverter()
        # result = converter.convert(path)
        # raw = result.document.export_to_markdown() or ""
        # lang = detect_language(raw) 등...
        # noise = calculate_noise_score(raw, file_type=path.suffix.lower().lstrip("."), is_ocr=use_ocr)
        # raw = noise["cleaned_text"]
        # moved = move_to_output(path)

        st.session_state["current_stage"] = "chunk"

        # ── 옵티마이저 실행 ──────────────────────────────────────────────
        doc_type     = path.suffix.lower().lstrip(".")
        chunk_result = optimize_chunks(raw, doc_type)   # raw = 정제된 텍스트
        chunks       = chunk_result.chunks

        # 폴백: 옵티마이저 결과 없으면 UI 파라미터로 단회성 청킹
        if not chunks:
            from core.chunking_optimizer import chunk_once
            chunk_result = chunk_once(raw, chunk_size, chunk_overlap)
            chunks       = chunk_result.chunks

        # 기존 write_outputs() (txt + 기본 md) 유지
        md_path, chunk_path = write_outputs(path, raw, chunks)

        # 최적화 MD 추가 저장
        stem = path.stem
        save_optimized_md(chunk_result, path.name, OUTPUT_DIR, stem)

        # 품질 메시지
        q = chunk_result.quality
        quality_msg = (f"✅ 품질 통과" if chunk_result.passed
                       else f"⚠️ 품질 미달 (noise={q.avg_noise:.1f}, dup={q.avg_dup:.2f})")
        ...
        return {
            ...
            "chunk_params": chunk_result.params,      # ← 신규
            "chunk_quality": quality_msg,             # ← 신규
        }
    except Exception as e:
        safe_log_exception(f"process_one_file failed for {path.name}: {e}")
        st.session_state["current_stage"] = "error"
        st.session_state["current_error"] = str(e)
...
def render_processing_tab(use_ocr: bool, chunk_size: int, chunk_overlap: int):
    try:
        ...
        if not files:
            ...
            return
        selected = st.multiselect("처리할 파일", [p.name for p in files], default=[p.name for p in files[:min(5, len(files))]])
        c1, c2 = st.columns(2)
        with c1:
        ...
        if start_parse:
            ...
            for idx, name in enumerate(selected, start=1):
                ...
                if path.exists():
                    ...
                    result = process_one_file(path, use_ocr=use_ocr, chunk_size=chunk_size, chunk_overlap=chunk_overlap)
