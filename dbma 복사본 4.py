...
def _apply_select_all(file_names: list[str]):
...
def _on_item_change(file_names: list[str]):
    sel_keys = [f"sel_{name}" for name in file_names if f"sel_{name}" in st.session_state]
    st.session_state["_select_all"] = all(st.session_state[k] for k in sel_keys) if sel_keys else False
...
with tab1:
    ...
    with col_left:
        ...
        if not file_list:
            ...
            st.session_state["_select_all"] = all(
                st.session_state.get(f"sel_{name}", False) for name in file_names
