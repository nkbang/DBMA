...
- [x] §2.1 착수 — 실제 `ui/app.py` 사이드바가 이미 내비게이션 형태였음을
      ...
      기술 용어 노출을 실제로 발견·수정
- [x] `ui/pages/*` P0 화면(홈/검색/도움말/내 자료) 점검 완료 — 목업 기준
      전면 재구성은 불필요함을 확인(§4.3, UX-002 §5.1). 대신 실제 갭이었던
      Sample Library를 `DBMA-UX-003-SAMPLE-LIBRARY-PLAN.md`로 구현,
      "보기"/"복사하여 내 자료로" 버튼까지 실 브라우저 클릭으로 검증 완료

**진행률: 100%** (Stitch 9화면 확보, C1 Review GO with caveats, UX-002/003
착수하여 라이브 앱의 기술 용어 노출 9건 수정 + Sample Library 구현·검증
완료. 남은 것은 향후 별도 트랙(P1/P2 화면, 마이크로 인터랙션 등)뿐)
