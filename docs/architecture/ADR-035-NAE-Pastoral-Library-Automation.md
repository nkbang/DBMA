...
| 인덱스 갱신 수동 트리거 → 파일 변경 감지 자동 갱신 | **이미 구현됨 — ADR-035 초안 작성 시 CUE가 놓친 항목, 구현 착수 직전 재확인하며 발견·정정(2026-09-23)** — 신규 작업 불필요 | `core/background_index_builder.py`(HQ 제안 ⑧, `docs/STATE.md:687`)가 백그라운드 데몬 스레드로 `reconcile_pending()`을 주기 실행(기본 5초) + `_execute_processing()` 성공 직후 `get_shared_background_builder().trigger_now()`로 즉시 깨움(`ui/pages/processing.py:679-683`). Pull 기반·idempotent라 동시 트리거에도 안전 — C1 Review RQ3가 지적한 "인덱스 갱신 동시성" 우려도 이미 이 설계로 해소되어 있었음 |
...
**"인덱스 자동 갱신"은 이 ADR에서 다루지 않는다** — §2에서 정정했듯
`core/background_index_builder.py`(HQ 제안 ⑧)로 이미 완전히 구현되어
있고, C1 Review(RQ3)가 지적한 동시성 우려도 그 pull 기반·idempotent
설계로 이미 해소돼 있다. 구현 착수 직전 재확인 과정에서 CUE가 초안
작성 시 놓쳤던 것을 발견해 정정했다(중복 구현 방지).
...
- 이 ADR로 새로 구현할 항목은 "RAW 폴더 워처(감지 알림)"와 "검색
  결과 자동 수집(옵트인)" 2건뿐이다. 파괴적 작업(삭제)은 여전히 사람이
  최종 확인하며, 인덱스 갱신은 이미 구현된 기존 자동화를 그대로 쓴다 —
  안전과 편의의 균형을 코드 실측 근거로 결정했다.
