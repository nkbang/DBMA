...
> **상태: 동결 해지 (2026-09-18, HQ 결정)** — 2026-09-15 "영구 취소·재개 금지" 결정은
> 철회되었다(`docs/STATE.md` 2026-09-18 항목 참고). **단, 이 해지는 "재개해도 된다"는
> 뜻이지 "Track A 또는 B를 지금 실행하라"는 승인이 아니다** — 아래 Phase 0~4는 여전히
> **미착수**이며, 어느 트랙으로 갈지 별도 HQ 지시가 필요하다. 또한 2026-09-18 확인
> 결과 이 문서의 "대상" 파일(`output/bench/tsu_dataset.jsonl`)이 동결 선언 당일
> (2026-09-15 15:23)에 이미 119,595건(Spurgeon 전용)으로 교체되어 있었다는 사실이
> 새로 드러났다 — 아래 §1.1의 "현재 1,363건" 수치는 **더 이상 실측과 일치하지 않는
> 과거 스냅샷**이다. 이 문서는 사고 경위·원인 사슬·백업 위치 기록용으로 계속 보존하되,
> 실행 여부를 결정하기 전에 반드시 `docs/STATE.md` 2026-09-18 항목의 최신 실측치부터
> 다시 확인할 것.
