...
div[data-testid="stFormSubmitButton"] > button {
    ...
    border-radius:8px!important; font-family:'Noto Serif KR',serif!important;
    font-weight:600!important; padding:8px 20px!important; font-size:0.90rem!important;
}
...
<div class="dbma-header">
  <h1>📚 DBMA 파싱 파이프라인</h1>
  <p>David Bang Ministry Archive · RAG 데이터 정제 시스템 v3.0</p>
</div>
""", unsafe_allow_html=True)
...
def load_md_noise(path: str, mtime_ts: float) -> dict:
    with open(path, encoding="utf-8") as fh:
        content = fh.read()
    ns = calculate_noise_score(content)
    fm = content[:500]
    lm = re.search(r"languages: *(.+)", fm)
    langs = lm.group(1).strip() if lm else "—"
    return {"content": content, "noise": ns, "langs": langs}
...
if "file_list_cache" not in st.session_state:
    st.session_state["file_list_cache"] = []
...
if "last_target_dir" not in st.session_state:
    st.session_state["last_target_dir"] = None
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown(
        "<div style='font-family:JetBrains Mono,monospace;font-size:0.72rem;color:#3fb950;padding:8px 10px;background:#0d2b1a;border-radius:6px;border:1px solid #1a4a2a;'>✓ 파싱 완료 후 원본 PDF는<br>자동으로 제련완성본 폴더로<br>이동됩니다.</div>",
        unsafe_allow_html=True,
    )

# 디렉토리 변경 시 캐시 갱신
if st.session_state["last_target_dir"] != target_dir:
    st.session_state["file_list_cache"] = scan_directory(target_dir)
    current_names = {f["name"] for f in st.session_state["file_list_cache"]}
    if not st.session_state["selected_names"]:
        st.session_state["selected_names"] = set(current_names)
    else:
        st.session_state["selected_names"] &= current_names
    st.session_state["last_target_dir"] = target_dir

if st.session_state["last_output_dir"] != output_dir:
    st.session_state["analysis_data"] = None
...
with tab1:
    ...
    with col_left:
        ...
        with ref_col:
            if st.button("🔄 새로고침", key="refresh_files"):
        ...
        file_list = st.session_state["file_list_cache"]
        ...
        if not file_list:
            ...
            st.markdown(f"""
            <div class="stat-row">
              <div class="stat-card"><div class="stat-label">총 파일 수</div><div class="stat-value blue">{len(file_list)}</div></div>
              <div class="stat-card"><div class="stat-label">총 용량</div><div class="stat-value orange">{fmt_size(total_size)}</div></div>
            </div>
            """, unsafe_allow_html=True)

            current_names = {f["name"] for f in file_list}
            selected_now = st.session_state["selected_names"] & current_names

            with st.form("file_select_form", clear_on_submit=False):
                top1, top2, top3 = st.columns([1, 1, 2])
                with top1:
                    select_all_submit = st.form_submit_button("전체 선택")
                with top2:
                    clear_all_submit = st.form_submit_button("전체 해제")
                with top3:
                    apply_selection = st.form_submit_button("선택 적용")

                working_selected = set(selected_now)
                if select_all_submit:
                    working_selected = set(current_names)
                elif clear_all_submit:
                    working_selected = set()

                st.markdown("""
                <div class="file-table-header">
                  <span>✓</span><span>파일명</span><span>크기</span><span>수정일</span><span>형식</span>
                </div>
                """, unsafe_allow_html=True)
                ...
                for f in file_list:
                    ...
                    with c1:
                        checked = st.checkbox(
                            "",
                            value=(f["name"] in working_selected),
                            key=f"form_sel_{f['name']}",
                    ...
                    if checked:
                        temp_selected_names.add(f["name"])

                if select_all_submit:
                    st.session_state["selected_names"] = set(current_names)
                    st.success(f"전체 선택 완료: {len(current_names)}개")
                elif clear_all_submit:
                    st.session_state["selected_names"] = set()
                    st.warning("전체 해제 완료")
                elif apply_selection:
                    st.session_state["selected_names"] = temp_selected_names
                    st.success(f"선택 상태 반영 완료: {len(temp_selected_names)}개")

            selected_files = [f for f in file_list if f["name"] in st.session_state["selected_names"]]
            ...
            if selected_files:
                ...
                st.markdown(
                    "<div style='font-size:0.77rem;color:#d29922;padding:7px 4px;font-family:JetBrains Mono,monospace;'>⚠️ 선택된 파일 없음</div>",
                    unsafe_allow_html=True,
                )
    ...
    with col_right:
        ...
        if start_btn and selected_files:
            ...
            def save_md(stem: str, text: str, noise: dict) -> str:
            ...
            for idx, f in enumerate(selected_files):
                ...
                try:
                    ...
                    md_path = save_md(stem, full_text, noise)
                    ...
                    if os.path.abspath(src_path) != os.path.abspath(dest_path):
                        if os.path.exists(dest_path):
                            logs.append({"cls": "log-warn", "msg": "  ⚠ 출력 폴더에 같은 PDF 이름이 이미 있어 이동 생략"})
                        else:
                    ...
                    logs.append({"cls": "log-err", "msg": f"  ✗ 오류: {type(e).__name__}: {e}"})
                    fail_count += 1
            ...
            if fail_count == 0:
                st.success(f"✅ 전체 {ok_count}개 완료 — 품질 분석 탭에서 결과를 확인하세요.")
            else:
...
with tab2:
    ...
    refresh_analysis = st.button("🔄 분석 새로고침", key="refresh_analysis")
    if refresh_analysis or st.session_state["analysis_data"] is None:
        scan_md_files.clear()
        md_files = scan_md_files(output_dir)
        if md_files:
            all_scores = []
            for mf in md_files:
                loaded = load_md_noise(mf["path"], mf["mtime_ts"])
                all_scores.append({
                    **mf,
                    "noise": loaded["noise"],
            ...
            st.session_state["analysis_data"] = all_scores
        else:
            st.session_state["analysis_data"] = []

    all_scores = st.session_state["analysis_data"]
    ...
    if not all_scores:
        ...
        st.markdown(f"""
        <div class="stat-row">
          <div class="stat-card"><div class="stat-label">파일 수</div><div class="stat-value blue">{len(all_scores)}</div></div>
          ...
          <div class="stat-card"><div class="stat-label">우수</div><div class="stat-value green">{good_count}</div></div>
          <div class="stat-card"><div class="stat-label">주의</div><div class="stat-value orange">{warn_count}</div></div>
          <div class="stat-card"><div class="stat-label">재정제 필요</div><div class="stat-value red">{bad_count}</div></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        for x in sorted(all_scores, key=lambda v: v["noise"]["total"], reverse=True):
            ...
            <div style="margin-bottom:8px;">
              <div style="display:flex;justify-content:space-between;font-family:'JetBrains Mono',monospace;font-size:0.73rem;margin-bottom:2px;">
                <span style="color:#c9d1d9;">📄 {x['name']} <span style="color:#8b949e;font-size:0.66rem;margin-left:8px;">[{x['langs']}]</span></span>
                <span style="color:{col};font-weight:600;">{sc:.1f} &nbsp;{noise_label(sc)}</span>
              </div>
              <div class="noise-bar-wrap"><div class="noise-bar" style="width:{bar_pct}%;background:{col};"></div></div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if sel_data is None:
            st.error("선택한 파일 정보를 찾지 못했습니다.")
            st.stop()

        ns = sel_data["noise"]
        stem = os.path.splitext(selected_md)[0]
        content = sel_data["content"]
        ...
        with col_a:
            ...
            <div class="analysis-card">
              <h4>종합 노이즈 점수</h4>
              <div style="font-size:2.8rem;font-weight:700;color:{col};font-family:'JetBrains Mono',monospace;line-height:1.1;">{sc:.1f}</div>
              <div style="font-size:0.82rem;color:{col};margin-top:4px;">{noise_label(sc)}</div>
              <div class="noise-bar-wrap" style="margin-top:12px;"><div class="noise-bar" style="width:{int(sc)}%;background:{col};"></div></div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:0.71rem;color:#8b949e;margin-top:10px;line-height:1.9;">
        ...
        with col_b:
            def mini_bar(label, val, max_val, color):
                ...
                return (
                    ...
                    f"<div style='display:flex;justify-content:space-between;font-family:JetBrains Mono,monospace;font-size:0.70rem;color:#8b949e;margin-bottom:2px;'>"
                    f"<span>{label}</span><span style='color:{color};'>{val:.1f} / {max_val}</span></div>"
                    f"<div class='noise-bar-wrap'><div class='noise-bar' style='width:{pct}%;background:{color};'></div></div></div>"
                )
        ...
        if chunks:
            st.markdown(f"""
            <div class="analysis-card">
              <h4>청크 미리보기 &nbsp;<span style='color:#8b949e;font-size:0.75rem;font-family:JetBrains Mono,monospace;font-weight:400;'>총 {len(chunks)}개</span></h4>
            </div>
            """, unsafe_allow_html=True)
        ...
        with st.expander("📄 MD 본문 전체 보기"):
            ...
            st.text_area("", body[:3000] + ("…" if len(body) > 3000 else ""), height=300, label_visibility="collapsed")