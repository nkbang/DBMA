...
with tab1:
    ...
    with col_left:
        ...
        if not file_list:
            ...
            for f in file_list:
                sel_key = f"sel_{f['name']}"
                if sel_key not in st.session_state:
                    st.session_state[sel_key] = st.session_state["_select_all"]
...
with tab2:
    ...
    if not md_files:
        ...
        with col_b:
            def mini_bar(label, val, max_val, color):
                ...
                return (
                    ...
                    f"<div class='noise-bar-wrap'><div class='noise-bar' style='width:{pct}%;background:{color};'></div></div>"
                    f"</div>"
                )
            ...
            <div class="analysis-card">
              <h4>세부 지표</h4>
              {mini_bar("특수문자", ns["symbol_ratio"], 40, "#f85149")}
              {mini_bar("짧은 줄", ns["short_line"], 30, "#d29922")}
              {mini_bar("빈 줄", ns["blank_ratio"], 15, "#58a6ff")}
        ...
        st.markdown("##### MD 본문 미리보기")
        st.text_area("본문", body[:3000] if len(body) > 3000 else body, height=300, label_visibility="collapsed")

        chunks = load_chunks_info(output_dir, stem)
        if chunks:
            st.markdown(f"""
            <div class="analysis-card">
              <h4>청크 미리보기 <span style="color:#8b949e;font-size:0.75rem;font-family:'JetBrains Mono',monospace;font-weight:400;">{len(chunks)}개</span></h4>
            </div>
            """, unsafe_allow_html=True)
            preview_n = st.slider("청크 개수", 1, min(len(chunks), 10), 3)
