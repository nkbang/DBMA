...
1) dbma.py의 출력 형식을 그대로 소비한다 (새 포맷을 만들지 않음).
   ...
   - {stem}_chunks.txt  : "[chunk N]" 구분자로 나뉜 청크 텍스트
...
_yaml_dirs = _CFG.get("directories", {})
DEFAULT_OUTPUT_DIR_YAML = _yaml_dirs.get("output_dir", "output")
...
APP_NAME    = "DBMA-RAG"
APP_VERSION = "0.2.3"   # 업그레이드: config.yaml 기반 동적 경로 적용

BASE_DIR            = os.path.dirname(os.path.abspath(__file__))

# config.yaml 기반 output_dir (폴더 기본값은 기존 호환성 유지)
if DEFAULT_OUTPUT_DIR_YAML:
    DEFAULT_OUTPUT_DIR = os.path.join(BASE_DIR, DEFAULT_OUTPUT_DIR_YAML)
else:
    # fallback: 기존 경로 유지
...
# [FIX v0.2.2] 원인1: 전역 컴파일 상수로 교체 (이중 이스케이프 오류 제거)
# 패턴1: [chunk N]   ← dbma.py 실제 출력 포맷
PAT_BRACKET = re.compile(r"(?m)^\\[chunk\\s+\\d+\\]\s*$")
# 패턴2: ════ CHUNK N/N ════  ← 구형 포맷
PAT_BOX     = re.compile(r"(?m)^═+\s*CHUNK\s+\d+\s*/\s*\d+\s*═+\s*$")
...
def parse_chunks_file(path: str) -> list[str]:
    """dbma.py 산출물의 여러 청크 포맷을 안정적으로 파싱."""
    with open(path, encoding="utf-8") as f:
        text = f.read().strip()

    chunk_patterns = [
        re.compile(r"(?m)^\\[chunk\\s+\\d+\\]\s*$"),
        re.compile(r"(?m)^═+\s*CHUNK\s+\d+\s*/\s*\d+\s*═+\s*$"),
    ]

    raw_chunks = None
    for pat in chunk_patterns:
        if pat.search(text):
            raw_chunks = pat.split(text)
            logger.info("parse_chunks_file: matched pattern=%s path=%s", pat.pattern, path)
            break

    if raw_chunks is None:
        raw_chunks = re.split(r"\n{2,}", text)
        logger.warning("parse_chunks_file: no header pattern matched, falling back to blank-line split path=%s", path)
    ...
    for idx, raw in enumerate(raw_chunks, 1):
        ...
        if length < 20:
            short += 1
            logger.warning("chunk[%d] skipped: short len=%d preview=%s", idx, length, preview)
            continue

        logger.info("chunk[%d] accepted len=%d preview=%s", idx, length, preview)
        cleaned.append(chunk)

    logger.info(
        "parse_chunks_file path=%s raw=%d accepted=%d empty=%d short=%d",
        path, len(raw_chunks), len(cleaned), empty, short,
    )
    return cleaned
    ...
    for doc in docs:
        ...
        if existing and existing.get("ids"):
            try:
                ...
                logger.warning("collection.delete failed for %s: %s", stem, e)
                logs.append(f"⚠️  {stem}: 기존 항목 삭제 실패, 인덱스 초기화 후 재시도 필요")
                continue
...
def reset_index(persist_dir: str) -> list[str]:
    """[FIX v0.2.2] 원인2 해결: chroma DB 디렉터리 전체 삭제 후 재생성 + 캐시 클리어."""
    logs = []
    if os.path.isdir(persist_dir):
        shutil.rmtree(persist_dir)
        logs.append(f"삭제: {persist_dir}")
    os.makedirs(persist_dir, exist_ok=True)
    logs.append("빈 디렉터리 재생성 완료")
    st.cache_resource.clear()
    logs.append("Streamlit 캐시 클리어 완료")
    logs.append("✅ 인덱스 완전 초기화 완료 — 이제 '전체 재색인' 버튼을 누르세요.")
    return logs
...
def main():
    ...
    # ── 탭 ───────────────────────────────────────────────────────
    tab1, tab2, tab3 = st.tabs(["① 색인 구축", "② 질의 (RAG)", "③ 디버그"])
    ...
    with tab1:
        ...
        # [FIX v0.2.2] 원인2 해결 버튼
        if c3.button("🗑️ 인덱스 초기화", use_container_width=True, type="secondary",
                     help="chroma DB와 상태파일을 모두 삭제하고 빈 DB를 재생성합니다."):
            with st.spinner("인덱스 초기화 중..."):
                reset_logs = reset_index(persist_dir)
            for line in reset_logs:
                st.write(line)
            st.info("이제 '전체 재색인' 버튼을 눌러 새로 색인하세요.")
    ...
    with tab3:
        st.subheader("🔧 디버그 도구")
        st.caption("[FIX v0.2.2] 청크 정규식 검증 · 인덱스 상태 점검 · 로그 확인")

        # ── 섹션 1: 청크 정규식 검증 ──────────────────────────────
        st.markdown("#### 1️⃣  청크 파일 정규식 검증")
        docs_found = discover_documents(output_dir) if os.path.isdir(output_dir) else []
        if not docs_found:
            st.warning(f"폴더에 _chunks.txt 파일이 없습니다: {output_dir}")
        else:
            sel = st.selectbox("청크 파일 선택", [d["stem"] for d in docs_found])
            sel_doc = next(d for d in docs_found if d["stem"] == sel)

            if st.button("🔎 정규식 매칭 검증"):
                with open(sel_doc["chunks_path"], encoding="utf-8") as f:
                    sample = f.read(600)

                bracket_hit = bool(PAT_BRACKET.search(sample))
                box_hit     = bool(PAT_BOX.search(sample))

                st.code(sample, language="text")
                col_a, col_b = st.columns(2)
                col_a.metric("패턴 [chunk N] 매칭", "✅ 일치" if bracket_hit else "❌ 불일치")
                col_b.metric("패턴 ════CHUNK════ 매칭", "✅ 일치" if box_hit else "❌ 불일치")

                chunks = parse_chunks_file(sel_doc["chunks_path"])
                if chunks:
                    st.success(f"✅ 파싱된 청크 수: {len(chunks)}개")
                    with st.expander("청크 예시 (첫 3개)", expanded=True):
                        for i, c in enumerate(chunks[:3], 1):
                            st.markdown(f"**[{i}]** {c[:300]}{'...' if len(c) > 300 else ''}")
                            st.divider()
                else:
                    st.error("❌ 청크가 0개입니다. 정규식 패턴을 점검하세요.")

        # ── 섹션 2: 벡터DB 상태 ───────────────────────────────────
        st.markdown("---")
        st.markdown("#### 2️⃣  벡터DB 상태")
        if st.button("📊 DB 상태 조회"):
            try:
                col = get_collection(persist_dir)
                cnt = col.count()
                st.success(f"벡터DB 색인 청크 수: {cnt}개")
                if cnt > 0:
                    peek = col.peek(limit=3)
                    for idx, (doc_text, meta) in enumerate(
                        zip(peek.get("documents", []), peek.get("metadatas", [])), 1
                    ):
                        st.markdown(f"**#{idx}** `{meta.get('source')} · 청크 {meta.get('chunk_index')}`")
                        st.text(doc_text[:200])
                        st.divider()
                else:
                    st.warning("DB는 존재하나 색인된 청크가 없습니다. '인덱스 초기화' 후 '전체 재색인'을 실행하세요.")
            except Exception as e:
                st.error(f"DB 조회 실패: {e}")

        # ── 섹션 3: 로그 확인 ─────────────────────────────────────
        st.markdown("---")
        st.markdown("#### 3️⃣  로그 확인 (최근 60줄)")
        if st.button("📄 로그 보기"):
            log_path = os.path.join(BASE_DIR, "logs", "dbma_rag_streamlit.log")
