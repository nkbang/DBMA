"""Ingest Smith Bible Dictionary Vol04 into Qdrant."""
import hashlib
import uuid
import time
...
vol = 4
canonical_path = Path(f"NAE/corpus/canonical/Smith_Bible_Dictionary_HackettAbbot_Vol{vol}/canonical.json")
identifier = f"Smith_Bible_Dictionary_HackettAbbot_Vol{vol}"
source_id = f"BAP-REF-SMITH-VOL{vol:02d}"
volume = f"vol_{vol}"

print(f"Starting Vol{vol}...")
start_all = time.time()
...
for i, chunk in enumerate(chunks):
    ...
    vector = embed_client.embed_text(chunk.text, content_hash=content_hash, model="bge-m3")
    if vector is None:
...
print(f"Done! Vol{vol}: {embedded} embedded, {len(errors)} errors in {elapsed:.1f}s")
