...
Phase 2: P1-first ordering (P1=176 -> batch_0001(100)+batch_0002(76), P2=batch_0003..0038)
Total: 38 batches, 3,643 TSU.

batch_id prefix: fuller_v01_batch_NNNN (0001-0038)
...
def main() -> None:
    ...
    if args.apply:
        ...
        for idx, (tier, batch) in enumerate(all_batches, 1):
            batch_id = f"fuller_v01_batch_{idx:04d}"
            reqs = build_requests_from_records(batch)
            path = write_batch_requests(reqs, batch_id)
            print(f"  Written: {path} ({len(reqs)} requests) [{tier}]")
