...
#
# Runs each volume via `NAE.pipeline.tsu.runner --identifier`. That path RESTARTS
# from candidate 0 on every invocation (no resume), so this script skips volumes
# whose tsu_report.json already shows "partial": false, and re-runs any volume
# left partial (overwrites cleanly; only cost is wasted TSU id space + compute).
#
...
for v in "${VOLS[@]}"; do
  ...
  if [ -f "$RPT" ] && "$PY" -c "import json,sys; sys.exit(0 if json.load(open('$RPT')).get('partial') is False else 1)"; then
    log "Vol${v}: already complete (partial=false) — skip"
    continue
  fi
  log "Vol${v}: starting ($ID)"
  "$PY" -m NAE.pipeline.tsu.runner --identifier "$ID" --model "$MODEL"
  rc=$?
