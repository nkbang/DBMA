...
def calculate_noise_score(text: str) -> dict:
    ...
    non_allowed = char_count - allowed_count
...
with tab2:
    ...
    if not md_files:
        ...
        with colb:
            ...
            <div class="analysis-card">
              <h4>노이즈 세부 구성</h4>
              {mini_bar("기호 노이즈", ns["symbol_ratio"], 40, "#f85149")}
              {mini_bar("짧은 줄 비율", ns["short_line"], 30, "#d29922")}
              {mini_bar("빈 줄 비율", ns["blank_ratio"], 15, "#58a6ff")}
              {mini_bar("반복 문자열", ns["repeat_ratio"], 15, "#a371f7")}
            </div>
            """, unsafe_allow_html=True)
