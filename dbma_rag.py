...
# ── 기본 경로 (스크립트 위치 기준 상대경로) ───────────────────────────────
# dbma.py와 같은 폴더에 두면 DEFAULT_OUTPUT_DIR이 dbma.py의 출력 폴더와 자동으로 일치함.
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DEFAULT_OUTPUT_DIR = os.path.join(BASE_DIR, "data", "제련완성본")
DEFAULT_PERSIST_DIR = os.path.join(BASE_DIR, "data", "rag_index")

# 검색 품질이 가장 좋다고 알려진 다국어(한/영/히브리어/헬라어 포함) 임베딩 모델.
# e5 계열은 "query: " / "passage: " 접두어를 붙여야 성능이 정상적으로 나옴 (모델 학습 방식).
EMBED_MODEL_NAME = "intfloat/multilingual-e5-large"
EMBED_USES_E5_PREFIX = True

# 한국어 instruction-following이 비교적 강한 Ollama 모델을 기본값으로 둠.
# 사전에 `ollama pull <모델명>`으로 받아둔 모델 이름을 그대로 사이드바에 입력하면 됨.
...
def build_index(output_dir: str, persist_dir: str, force: bool = False) -> list[str]:
    ...
    for doc in docs:
        ...
        # 기존 청크(개수가 줄어든 경우 등 포함) 제거 후 재삽입
...
def retrieve(question: str, persist_dir: str, top_k: int, exclude_noisy: bool, noise_threshold: float):
    embedder = get_embedder()
    collection = get_collection(persist_dir)
...
with st.sidebar:
    ...
    noise_threshold = st.slider("노이즈 제외 기준 (이상이면 제외)", 0, 100, 40, disabled=not exclude_noisy)
    ...
    if st.button("🔌 Ollama 연결 확인"):
        try:
            ...
            st.success(f"연결 성공. 로컬에 받아둔 모델: {', '.join(names) if names else '없음'}")
...
with tab1:
    ...
    if docs:
        st.dataframe(
            [{"문서": d["stem"], "언어": d["languages"], "노이즈": d["noise_score"],
              "상태": d["noise_status"]} for d in docs],
    ...
    if c2.button("♻️ 전체 재색인", use_container_width=True):
        with st.spinner("전체 재색인 중... (문서 수에 따라 시간이 걸립니다)"):
...
with tab2:
    ...
    question = st.text_area("질문을 입력하십시오", height=100,
        ...
        if not os.path.isdir(persist_dir):
            ...
            if not hits:
                ...
                if not ollama_model.strip():
                    st.warning("사이드바에 Ollama 모델 이름을 입력하면 답변까지 생성됩니다. (현재는 검색 결과만 표시)")
                else:
                    with st.spinner(f"{ollama_model} 로 답변 작성 중... (로컬 실행이라 모델 크기에 따라 시간이 걸릴 수 있습니다)"):
                        try:
