...
# - 전체 선택 / 해제 버그 수정
# - 캐시 비우기 / 새로고침 연동
# - Cleaner Engine 1차 통합
# - 페이지 번호 제거
# - 하이픈 줄바꿈 복원
# - OCR 잡음 기호 제거
# - 과도한 공백 정규화
# - Header/Footer 보수 제거 옵션 추가
# - footnote ratio 메타데이터 추가
# - pipeline 버전 표기 APP_VERSION 연동
# - 품질 분석 탭 재파싱 기능 유지
# - langs_found NameError 제거
# - PDF 이동 시 동일 경로 / 중복 파일 방지
...
with tab1:
    ...
    with col_right:
        ...
        <div style="background:#161b22;border:1px solid #21262d;border-radius:8px;
          ...
          <span style="color:#8b949e;">CLEANER     </span>
          <span style="color:#3fb950;">기본 정제 활성화</span><br>
          <span style="color:#8b949e;">HDR/FTR     </span>
          <span style="color:#{'d29922' if aggressive_headers else '8b949e'};">{'보수 제거' if aggressive_headers else '비활성화'}</span><br>
...
with tab2:
    ...
    if not md_files:
        ...
        re_aggressive_headers = st.checkbox("재파싱 시 Header/Footer 보수 제거", value=aggressive_headers, key="reparse_headers")
        ...
        if st.button(f"선택 파일 재파싱 ({len(analysis_sel)}개)", disabled=(len(analysis_sel) == 0)):
            ...
            if not reparse_targets:
                ...
                result = run_parsing_batch(
                    ...
                    aggressive_headers=re_aggressive_headers,
            ...
            with col_a:
                ...
                <div class="analysis-card">
                  ...
                  <div style="font-family:'JetBrains Mono',monospace;font-size:0.71rem;
                    ...
                    각주 비율: {ns.get('footnote_ratio', 0.0):.4f}<br>
